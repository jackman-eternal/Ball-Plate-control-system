...
/************************外部变量声明********************/
 ...
 u16 time = 0;													//用来寄存时间
volatile u16 PASSBY_TIME = 0;										//连续接近目标位置的次数 
	...
	void TIM4_Int_Init(u16 arr,u16 psc)
{
	...
	TIM4->ARR = arr;					//Timer2 重装载值
	TIM4->PSC = psc;					//Timer2 预分频值
	
	TIM4->DIER |= 1<<0;				//定时器更新中断
	TIM4->CR1 |= 0x0080;			//ARPE enable自动装载
...
void TIM4_IRQHandler(void)   //TIM4中断
{
		...
		//以下语句用来统计当前球的位置是不是连续都在目标区域内部，如果有一次不进入
...
//		if(Flag_Target == 1)
//		{	
//			USART_Cmd(USART3, ENABLE);                    //使能串口 采集数据
//		}
		if(Flag_Target == 1)                       //计算 pid     
		{
